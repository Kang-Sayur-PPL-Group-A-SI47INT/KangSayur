<x-app-layout>
    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <a href="{{ route('customer.offers') }}" class="inline-flex items-center gap-1.5 text-green-700 hover:text-green-800 text-sm font-medium mb-6 group">
            ...
            <div class="lg:col-span-1 space-y-4">
                <div class="bg-white rounded-2xl border border-gray-100 p-5">
                    ...
                    <div class="space-y-3 text-sm">
                        <div>
                            <p class="text-gray-400 text-xs uppercase tracking-wider">Farmer</p>
                            <p class="font-semibold text-gray-900 mt-0.5">{{ $offer->listing->farmer->name ?? '' }}</p>
                        </div>
                        <div>
                        ...
                        <div class="border-t border-gray-100 pt-3">
                            <p class="text-gray-400 text-xs uppercase tracking-wider">Your Offer</p>
                            <p class="font-bold text-green-700 text-lg mt-0.5">Rp {{ number_format($offer->offered_price, 0, ',', '.') }}</p>
                        </div>
                        @if($offer->counter_price)
                        <div>
                            <p class="text-gray-400 text-xs uppercase tracking-wider">Farmer's Counter</p>
                ...
                <div class="bg-white rounded-2xl border border-gray-100 p-5 space-y-3">
                    @if($offer->status === 'countered' && $offer->counter_price)
                        <form method="POST" action="{{ route('customer.offers.acceptCounter', $offer) }}">
                            @csrf
                    ...
                    <form method="POST" action="{{ route('customer.offers.update', $offer) }}" class="space-y-2">
                        @csrf
                        @method('PUT')
                        <input type="number" name="offered_price" placeholder="Update your offer (Rp)" required min="1"
                            class="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:border-green-400 focus:ring-2 focus:ring-green-100 outline-none text-sm transition-all">
                        <button class="w-full py-2.5 bg-blue-600 text-white font-semibold rounded-xl hover:bg-blue-700 transition-all duration-200 text-sm">↩ Update Offer</button>
                    </form>

                    <form method="POST" action="{{ route('customer.offers.destroy', $offer) }}" onsubmit="return confirm('Are you sure you want to withdraw this offer?')">
                        @csrf
                        @method('DELETE')
                ...
                <div class="bg-white rounded-2xl border border-gray-100 relative" style="height: 600px;">
                    ...
                    <div class="absolute top-0 left-0 right-0 px-5 py-4 border-b border-gray-100 flex items-center gap-3 bg-white rounded-t-2xl z-10">
                        <div class="w-9 h-9 rounded-full bg-gradient-to-br from-green-400 to-emerald-500 flex items-center justify-center text-white font-bold text-sm flex-shrink-0">
                            {{ strtoupper(substr($offer->listing->farmer->name ?? '', 0, 1)) }}
                        </div>
                        <div class="min-w-0">
                            <h3 class="font-semibold text-gray-900 text-sm">{{ $offer->listing->farmer->name ?? 'Farmer' }}</h3>
                    ...
                    @if(in_array($offer->status, ['pending', 'countered']))
                    <form method="POST" action="{{ route('customer.offers.message', $offer) }}" class="absolute bottom-0 left-0 right-0 px-4 py-3 border-t border-gray-100 bg-white rounded-b-2xl z-10">
                        @csrf
